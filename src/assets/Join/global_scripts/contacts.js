...
async function loadContactList() {
    ...
    if (contactResults) {
        ...
        firstLettersArray.forEach((letter) => {
            ...
            const contactsWithSameFirstLetter = contactsArray.filter(
                (contact) => contact.name[0] == letter
            );
            contactsWithSameFirstLetter.forEach((contact) =>
                renderContactInList(contact)
            );
        });
...
function getFirstLettersArray(contactsArray) {
    ...
    contactsArray.forEach((contact) => {
        ...
        !firstLettersArray.includes(firstLetter) &&
...
function toggleRespContextMenu(contact) {
    ...
    if (buttonRef.classList.contains("dnone")) {
        setTimeout(
            () => document.addEventListener("click", closeRespContextMenuCheck),
            0
        );
        document.getElementById("respEditButton").onclick = () =>
            openOverlay("editOverlayContainer", "editContactCardOverlay", contact);
        document.getElementById("respDeleteButton").onclick = () => {
...
function openOverlay(containerRefID, cardRefId, contact) {
    ...
    document
        .getElementById(containerRefID)
        .classList.add("overlayBackgroundColor");
    window.innerWidth > 1050
        ? document.getElementById(cardRefId).classList.add("slideInRight")
        : document.getElementById(cardRefId).classList.add("slideInBottom");
...
function closeOverlay(containerRefID, cardRefId) {
    document
        .getElementById(containerRefID)
        .classList.remove("overlayBackgroundColor");
...
function loadEditContactCard(contact) {
    ...
    document
        .getElementById("editContactButton")
        .setAttribute("onclick", `editContact(${JSON.stringify(contact)})`);
    document
        .getElementById("deleteContactOverlayButton")
        .setAttribute("onclick", `deleteContact("${contact.id}")`);
...
function emptyInputFields() {
    ...
    document
        ...
        .forEach((errorRef) => removeErrorMessage(errorRef));
}
...
function hardcloseEditOverlay() {
    document
        .getElementById("editOverlayContainer")
        .classList.remove("overlayBackgroundColor");
    document
        .getElementById("editContactCardOverlay")
        .classList.remove("slideInRight");
    document
        .getElementById("editOverlayContainer")
        .classList.remove("overlayAppear");
}
...
async function deleteContact(key) {
    ...
    if (
        document
            .getElementById("editOverlayContainer")
            .classList.contains("overlayAppear")
    ) {
        document
            .getElementById("editOverlayContainer")
            .classList.remove("overlayBackgroundColor");
...
function contactCreatedSuccess() {
    ...
    const slideInDirectionClass =
        window.innerWidth > 1050 ? "slideInRight" : "slideInBottomSuccess";
    document
        .getElementById("addContactOverlayContainer")
        .classList.remove("overlayBackgroundColor");
    document
        .getElementById("addContactCardOverlay")
        .classList.remove("slideInRight");
    document
        .getElementById("addContactOverlayContainer")
        .classList.remove("overlayAppear");
    ...
    setTimeout(() => {
        ref.classList.remove(slideInDirectionClass);
    }, 800);
}
